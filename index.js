...
for (const file of commandFiles){
    ...
    //verificar se existe o com,mando
    if ("data" in command && "execute" in command){

        client.commands.set(command.data.name, command)

    } else {
        console.log(`Esse comando em ${filePath} do bot ${c.user.tag} esta com "data" ou "Execute ausente"`);
    }
}
...
client.on(Events.InteractionCreate, async interaction => {
    ...
    try {
        ...
        console.error(error)
        await interaction.reply("Falhou miseralvemente ao executar esse comando!")
    }
